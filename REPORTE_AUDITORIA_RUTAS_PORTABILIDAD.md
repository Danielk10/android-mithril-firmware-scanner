# Reporte de Auditoría y Corrección: Rutas y Portabilidad de Mithril Firmware Scanner
ESTADO: CORREGIDO (Septiembre 2026)

Este documento detalla los hallazgos críticos y las acciones correctivas aplicadas para garantizar la portabilidad de las herramientas nativas (mithril, bsdunzip, libarchive, etc.) en Android para el paquete `com.diamon.mithril`.

### 1. Problema: Rutas Hardcoded en Binarios (RUNPATH) - SOLUCIONADO
**Acción:** Se utilizó `patchelf --remove-rpath` en todas las librerías compartidas y binarios dentro de `app/src/main/jniLibs/arm64-v8a/`. Durante la auditoría se detectaron referencias hardcodeadas a rutas de compilación obsoletas (ej. `/data/data/com.termux/files/usr/lib` y `/data/data/com.diamon.mithril/files/usr/lib`) incrustadas en la sección dinámica ELF `RUNPATH`.
...
**Resultado:** 
* Binarios ejecutables renombrados: `mithril` -> `libmithril_bin.so`, `bsdunzip` -> `libbsdunzip.so`.
* Librerías versionadas renombradas a formato seguro: `libarchive.so.13` -> `libarchive_13.so`, `libcrypto.so.3` -> `libcrypto_3.so`, `libicudata.so.78` -> `libicudata_78.so`, `libicuuc.so.78` -> `libicuuc_78.so`, `libxml2.so.16` -> `libxml2_16.so`, `liblzma.so.5` -> `liblzma_5.so`, `libz.so.1` -> `libz_1.so`, `libzstd.so.1` -> `libzstd_1.so`.
