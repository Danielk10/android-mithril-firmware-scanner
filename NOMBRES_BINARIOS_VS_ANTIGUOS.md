# Nombres Nuevos vs Antiguos (Binarios Ejecutables) - Mithril Firmware Scanner
...
|---|---|---|
| `mithril` | `libmithril_bin.so` | `usr/bin/mithril` |
...
Todas las dependencias listadas han sido verificadas dentro de `jniLibs/arm64-v8a/`. Las librerías que originalmente tenían versionado (como `.so.13`, `.so.78`, `.so.16`, `.so.3`) han sido renombradas con sufijos seguros como `_13.so`, `_78.so`, etc., y la app restaura sus SONAMEs originales como enlaces simbólicos en `usr/lib/`.

| Binario / Librería | Dependencias Exigidas (DT_NEEDED) | Estado en `arm64-v8a` / App |
|---|---|---|
| **`mithril`** | `libc.so`, `libc++_shared.so`, `libdl.so`, `libm.so` | ✅ Todas las dependencias presentes. |
...
2. **Documentación / Man pages:** Ubicadas en `app/src/main/assets/data/data/com.diamon.mithril/files/usr/share/man/`.
