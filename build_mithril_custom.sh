...
# ==========================================
echo "Descargando código fuente de mithril..."
rm -rf "$HOME/mithril"
git clone --depth 1 --recurse-submodules https://github.com/nmatt0/mithril.git "$HOME/mithril"

mkdir -p "$HOME/mithril/build"
cd "$HOME/mithril/build" || exit 1
...
# ==========================================
echo
echo "=== Compilación de mithril Exitosa ==="
if [ -f "$FAKE_USR/bin/mithril" ]; then
    ls -lh "$FAKE_USR/bin/mithril"
    readelf -d "$FAKE_USR/bin/mithril" | grep -E "(NEEDED|RPATH|RUNPATH)" || true
    readelf -l "$FAKE_USR/bin/mithril" | grep LOAD || true
fi
